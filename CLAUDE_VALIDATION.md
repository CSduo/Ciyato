...
| `testDebugUnitTest` | PASS — **154 tests, 0 failures, 0 errors**, verified 22 Aug | plus 40 regressions added since (backup outcome, focus lifetime, search date ranges, breach-response parsing, photo month bucketing) |
| `lintDebug` | PASS (0 errors, 141 warnings) — re-verified 22 Aug after B25 | **2 errors at baseline.** `NewApi` windowLightNavigationBar moved to `values-v27`. `QueryAllPackagesPermission` is suppressed on that single manifest element with a written rationale — a launcher is a documented exception, and the Play declaration is recorded in PLAY_RELEASE_EVIDENCE.md. Correction: an earlier revision of this row claimed PASS *before* the second error was actually handled; lint was still failing the build at that point. 142 warnings remain, untriaged. |
