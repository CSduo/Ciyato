...
| `testDebugUnitTest` | PASS — **214 tests, 0 failures, 0 errors**, verified 22 Aug | plus 40 regressions added since (backup outcome, focus lifetime, search date ranges, breach-response parsing, photo month bucketing) |
...
| CI end-to-end | **REPAIRED (B45)** — every step set `working-directory: ciyato-android`, a folder that does not exist in this repo, so the workflow failed before reaching Gradle on every single push since the baseline. Replaced by `android-ci.yml`, which runs from the actual root and gates on unit tests, `lintDebug` (fails on errors), `assembleDebug` **and `bundleRelease`**. Awaiting first green run on GitHub. |
